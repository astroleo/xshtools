...
##    - do not combine segments of flagged spectrum with line: use some clever edge detection to find segments?
##
...
def plotSL(id,pdf=False,synRange=True,plotSFH=True):
	dir=os.getenv('PROJECTS')+'/LP-BAT/XSHOOTER/STARLIGHT/STARLIGHTv04/'
	os.chdir(dir)
	...
	spec_obs=np.genfromtxt(file_obs, dtype={'names': ('wave','flux','noise','flag'),
		'formats': ('f8','f8','f8','f3')})
	spec_syn=np.genfromtxt(file_syn, dtype={'names': ('wave','fobs','fsyn','weight'),
		'formats': ('f8','f8','f8','f3')})
	masks=np.genfromtxt(file_masks, dtype={'names': ('lam1','lam2'),
		'formats': ('f8','f8')})
	...
	###
	if synRange == True:
		Xcfg="/Users/leo/Projekte/LP-BAT/XSHOOTER/STARLIGHT/STARLIGHTv04/XSHOO.in"
		w_min=0
		w_max=0
...
def plot_popvec_hist():
	dir=os.getenv('PROJECTS')+'/LP-BAT/XSHOOTER/STARLIGHT/STARLIGHTv04/out_dir/'
	os.chdir(dir)
